...
class ProbabilityMetrics:
    ...
    # Expected value
    expected_pnl_expiry: float  # EV at expiration (dollars)
...
def calculate_probability_metrics(
    ...
    # Expected PnL at expiration
    # EV = P(win) * max_profit - P(lose) * max_loss
    # Simplified for defined risk: use PoP
    expected_pnl = (
...
def _calculate_pop_expiry(
    ...
    is_credit = structure.entry_credit is not None and structure.entry_credit > 0
    
    if structure_type in (StructureType.CREDIT_SPREAD, StructureType.IRON_CONDOR):
        # Credit spread: profit if S_T stays away from short strike
        # Use breakeven for simplicity
    ...
    elif structure_type == StructureType.DEBIT_SPREAD:
    ...
    elif structure_type in (StructureType.BUTTERFLY, StructureType.IRON_BUTTERFLY):
        # Butterfly: profit in narrow range around center
        # Approximate as P(BE1 < S_T < BE2)
        if len(structure.breakevens) >= 2:
            be_low, be_high = min(structure.breakevens), max(structure.breakevens)
